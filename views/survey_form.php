...
<body>
        ...
        <?php if (! empty($survey) && $survey instanceof Survey): ?>
          <form id="survey_form" action="survey_form.php" method="post">
            ...
            <div class="input_form big widelabels">
              ...
              <?php $preguntas = count($survey->questions); ?>
              <?php foreach ($survey->questions as $i => $question): ?>
	              <div id="question<?php echo $question->question_id ?>" style="display: none;">
	                ...
	                <?php foreach ($question->choices as $j => $choice): ?>
	                  <div>
	                    ...
	                    <input id="<?php echo $question_html_id; ?>" type="<?php echo htmlspecialchars($question->question_type); ?>" name="question_id[<?php echo htmlspecialchars($question->question_id); ?>][]" value="<?php echo htmlspecialchars($choice->choice_text); ?>" onclick="abrirsiguiente(<?php
	                    	if ($preguntas == $question->question_id + 1)
	                    		{ echo $question->question_id + 1;
	                    		}else{
	                    		 echo 0; 
              ...
              <div class="submit_button" >
                <button id="submitButton" name="submitButton" style="display: none;">Enviar encuesta</button>
              </div>
            </div>
          </form>
        <?php endif; ?>
  ...
  <script type="text/javascript">
  	window.onload = validarprimera();


  	function validarprimera(){
  		var $primera = $('#question1');
  		$primera.show();
  	}
  	function abrirsiguiente(sig, ant){

  		if (sig > 0)
  		{
  			var $sig = '#question'+ sig;
	  		var $ant = '#question'+ ant;
	  		var $siguiente = $($sig);
	  		var $anterior = $($ant);
